...
```{r}
first_eight <- bench_results_indiv %>% 
  mutate(model = model_name %>% 
           str_replace_all("_", " ")) %>% 
  group_by(model) %>% 
...
```{r}
readRDS(here("Simulation Results", "simple-models", "continuous", "exponential", "gamma-prior", "exponential-gamma-accuracy.rds"))
