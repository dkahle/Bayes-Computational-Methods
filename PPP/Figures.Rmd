...
log_df <- tibble(x = seq(-4,4,length.out = 100), d_norm = dnorm(x), log_d_norm =  - log(d_norm))

log_df %>% ggplot(aes(x,log_d_norm)) + 
  geom_line(size = 2) + 
  theme_minimal() + 
  theme(
    axis.title = element_blank(),
...
set.seed(1)
n <- 1000
df <- tibble(x = rnorm(n), y = rnorm(n, sd = 3)) 
big_n <- 100000
full_df <- tibble(x = rnorm(big_n), y = rnorm(big_n, sd = 3)) 


full_df %>% 
  ggplot() + 
  stat_density_2d(aes(x, y, fill = ..level..), geom = "polygon", alpha = 0.5) + 
...
<!-- ```{r} -->
<!-- full_df_with_data <- full_df %>%  -->
<!--   mutate(x_data = rep(df$x[1], 100000)) %>%  -->
<!--   mutate(y_data = rep(df$y[1], 100000))  -->
...
<!-- ```{r} -->
<!-- full_df_with_data <- full_df %>%  -->
<!--   mutate(x_data = c(df$x[3], rep(df$x[1], 100000 - 1))) %>%  -->
<!--   mutate(y_data = c(df$y[3], rep(df$y[1], 100000 - 1))) -->
...
```{r}
full_df_with_data <- full_df %>% 
  mutate(x_data = c((df$x / 1.7), rep(0, (100000 - 1000)))) %>% 
  mutate(y_data = c((df$y / 1.7), rep(0, (100000 - 1000))))
