...
In Bayesian statistical inference, we are interested in finding the posterior distribution of a certain parameter, given a prior distribution and a likelihood for that parameter from a sample of data. Bayes Rule states that, given events $A$ and $B$, we can find the conditional probability of $A$ given $B$ using $$ P(A | B) = \frac{P(B|A)}{P(B)}.$$ More formally, we can derive the posterior distribution of a parameter $\theta$, $h(\theta | x)$, by $$h(\theta | x_1, x_2, \dots, x_n) = \frac{\ell(\theta | x_1, x_2, \dots, x_n) \pi(\theta)}{\int{\ell(\theta | x_1, x_2, \dots, x_n) \pi(\theta) d \theta}},$$ where $\ell(x | \theta)$ is proportional to the likelihood of $\theta$, given the data, and $\pi(\theta)$ is the prior distribution of $\theta$. For example, suppose that $X_i \sim Bern(\theta)$, and we are interested in inference on $\theta$. A reasonable prior to assign to $\theta$ would be a $Beta(\alpha, \beta)$, distribution, given that $\theta$ is constrained to be between 0 and 1, which is a property of the Beta distribution. Therefore, given that $\theta \sim Beta(\alpha, \beta)$ for some known values of $\alpha$ and $\beta$, then 
\begin{align*}
  h(\theta | x) &= \frac{\ell(\theta | x_1, x_2, \dots, x_n) \pi(\theta)}{\int{\ell(\theta | x_1, x_2, \dots, x_n) \pi(\theta) d \theta}} \\
...
\end{align*}
which is the kernel of a $Beta\left(\sum (x_i) + \alpha, n - \sum (x_i) + \beta \right)$ distribution. Therefore, we can analytically calculate the posterior distribution of $\theta$ as $$h(\theta | x) \sim Beta(\sum (x_i) + \alpha, n - \sum (x_i) + \beta).$$

In more complicated scenarios, it is not easy to calculate the posterior distribution of a parameter by hand, and in many cases it is impossible. This is where computing can help us. If we could somehow computationally grab samples from the posterior distribution we are interested in, then we could obtain most of the statistics that we are interested in. Markov Chain Monte Carlo (referred to as MCMC), is a common approach that allows for drawing samples of a parameter from a distribution that is approximately equivalent to the target posterior distribution of that parameter. An essential property of any Markov chain is that the distribution of each draw only depends on the most recent value drawn before it. As we keep obtaining draws, each draw is probabilisticly more likely to have come from the target posterior distribution. Based on the convergence properties of MCMC, we can expect each draw to be coming from the real distribution of the parameter we are interested, once we have let the Markov chain run long enough. 
